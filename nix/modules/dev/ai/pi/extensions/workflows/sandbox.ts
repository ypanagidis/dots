...
/**
 * Pi runs extensions under Bun, but the workflow child relies on Node's
 * permission model. Nix supplies an immutable Node executable for that child;
 * direct Node consumers can continue to use their current executable.
 */
function workflowNodeExecutable(): string {
  const configured = process.env.PI_WORKFLOW_NODE?.trim();
  if (configured !== undefined && configured !== "") return configured;
  if (process.allowedNodeEnvironmentFlags.has("--permission")) return process.execPath;
  throw new Error(
    "This runtime cannot enforce workflow child permissions; configure PI_WORKFLOW_NODE with a Node executable that supports --permission",
  );
}
...
export function runWorkflowSandbox(options: RunWorkflowSandboxOptions) {
  ...
  try {
    nodeExecutable = workflowNodeExecutable();
  } catch (error) {
